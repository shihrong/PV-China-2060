...
axs = axs.reshape(len(var_names), 2)  # 保证 2 维索引
...
for i, var_name in enumerate(var_names):
    ...
    for j in range(2):
        ...
        if j == 0:
            f = h5py.File('GHI_PV_v2.hdf5', 'r')  
        else:
            f =  h5py.File('GHI_PV_2060_MRI.hdf5', 'r')

        if i == 0: # PV
            tmp = (
                f[f'{var_name}_hr_avg'][:]
                - f[f'{var_name}_bs_avg'][:]
            )/1000*100
        else: # GHi
            tmp = (
                f[f'{var_name}_hr_avg'][:]
        ...
        for region_name,grid_id in egrid.items():
            ...
            data[f'{region_name}_{titles[j]}'] = np.nanmean(tmp[mask])
    ...
    for j in range(2):
        ...
        plot_province_map(region_colors,ax=ax,title=titles[j],fig_title = title1,mean_value=mean_value1)
        # 画南海
        plot_southsea(ax=ax)
...
plt.savefig(f'./figures/FigS8_map_MRI_diff.png', bbox_inches='tight', dpi=600)

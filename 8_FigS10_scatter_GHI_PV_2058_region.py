...
f2 = h5py.File("GHI_PV_2058.hdf5", "r")
...
plot_scatter(
    ...
    dghi_ens,
    dghi_mri,
    xlabel="2059-2061 mean $\Delta$GHI (W m$^{-2}$)",
    ylabel="the year of 2058 $\Delta$GHI (W m$^{-2}$)",
...
plot_scatter(
    ...
    dpv_ens,
    dpv_mri,
    xlabel="2059-2061 mean $\Delta$PVPOT (%)",
    ylabel="2058 $\Delta$PVPOT (%)",
    title_str="(b) $\Delta$PV",
)
axs[1].set_title("(b) $\Delta$PV", fontsize=8, fontweight="bold")

# 保存图片
plt.savefig("./figures/FigS9_scatter_regional_with_R2_2058.png", bbox_inches="tight", dpi=600)
plt.close()
